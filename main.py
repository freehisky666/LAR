...
if __name__ == '__main__':
    ...
    parser.add_argument('--skill-emb-dim', type=int, default=30)
    parser.add_argument('--answer-emb-dim', type=int, default=30)
    ...
    for dataset_set_index in range(5):
        ...
        #net = KT_backbone(256, 96, 80,  params.n_skill)
        net=net.to(device)
        
        # optimizer
        optimizer = optim.Adam(net.parameters(), lr=params.lr,weight_decay=1e-5)
        ...
        if "pid" not in params.data_name:
            dat = DATA(n_question=params.n_skill,
                       seqlen=params.seqlen, separate_char=',', maxstep=200)
        else:
        ...
        for epoch in range(params.max_iter):
            ...
            for idx in range(train_N):
                ...
                #skill = torch.where(skill==-1, torch.tensor([0]), skill)
